 ...
 */
export function isRef(node: object): boolean {
 ...
 * @return the modified (annotated) node
 */
export function walkObject(node: object, objectCallback: ObjectVisitor): JsonNode {
  return walkObj(node);

  function walkObj(node: object): JsonNode {
